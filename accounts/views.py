...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'Por favor activa tu cuenta en Tienda Para todos'
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.success(request, 'has iniciado seccion exitosamente')
...
def logout(request):
    auth.logout(request)
    messages.success(request, 'ha cerrado seccion')
    return redirect('login')
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.error(request, 'la activacion es invalidad, Contacte con el proveedor')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request, 'El Password se reteo correctamente')
            return redirect('login')
        else:
            messages.error(request, 'El Password de confirmacion no concuenda')
            return redirect('resetPassword')
    else:
...
def my_orders(request):
    ...
    return render(request, 'accounts/my_orderes.html', context)
...
def change_password(request):
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            messages.error(request, 'El Password no coincide con la confirmacion del Password')
            return redirect('change_password')
